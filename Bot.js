...
client.on('ready', async () => {
    client.user.setActivity(`🌊pikabots🌊`)
...
client.on("message", async (msg) => {
    if (msg.guild.id !== "979019982488887357") return
    const prefix = "-"
    ...
    switch (cmd) {
        ...
        if(devmode == true && !check("MANAGE_MESSAGES")&& !Number(msg.channel.id) == "979666749618286613"){
            return error("You Dont Have Permission To Do That")
        }
        ...
        case `bot${devmode == true? "dev" : ""}`:
            ...
            if(devmode == true && !check("ADMINISTRATOR") && !Number(msg.channel.id) == "979666749618286613"){
                return error("You Dont Have Permission To Do That")
            }
            if (!cmddata.get(`${msg.member.user.id}${msg.guild.id}Tier`) || cmddata.get(`${msg.member.user.id}${msg.guild.id}Tier`) < 1) return error("Bot Start", `You Dont Have Permissions To Bot!`)
...
client.login("OTc5NjY2NzQ5NjE4Mjg2NjEz.Gi54JG.2yYALcW26DCHtkPSWmY3qAJ9NxS09nVZmIeWjA");